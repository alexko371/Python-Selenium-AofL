...
def test_create_annual_subscription_boy_and_girl(app):
    ...
    app.sign_up.submit_and_confirm_email("ak001@dk.test")  ##### Change Email Credentials!
    ...
    app.onboarding.create_child_profile(child_name="Tim", gender="M", academic_level="first-grade")
    ...
    ##### (grade Levels' IDs: [toddler-time] [preschool] [pre-k] [kindergarten] [first-grade] [second-grade]
    app.onboarding.create_child_profile(child_name="Sansa", gender="F", academic_level="preschool")
...
def test_create_annual_subscription_girl_and_boy(app):
    app.sign_up.open_abcmouse_web_page("https://dk.qtest.abcmouse.com/abt/subscription")  # Update URL QTEST!
    app.sign_up.submit_and_confirm_email("ak03@dk.test")  ##### Change Email Credentials!
    app.sign_up.submit_and_confirm_password("test123")
    app.payment_info.submit_cc_qtest(cardholder="Test Alex Ko", code_cvv="1234", zip_code="12345")
...
def test_create_annual_subscription_with_assessment_girl_and_boy(app):
    app.sign_up.open_abcmouse_web_page("https://bd.qtest.abcmouse.com/abt/subscription")  # Update URL QTEST!
    app.sign_up.submit_and_confirm_email("ak3@bd.test")  ##### Change Email Credentials!
    app.sign_up.submit_and_confirm_password("test123")
    app.payment_info.submit_cc_qtest(cardholder="Test Alex Ko", code_cvv="1234", zip_code="12345")
    ...
    app.onboarding.input_parent_name(parent_first_name="AK Test", parent_family_name="Ko Test")
    ##### (grade Levels' IDs: [toddler-time] [preschool] [pre-k] [kindergarten] [first-grade] [second-grade]
    app.onboarding.create_child_profile(child_name="Gloria", gender="F", academic_level="pre-k")
    app.onboarding.choose_avatar(avatar="girl_avatar11")  #####  girl_avatar01  |  girl_avatar16
    app.onboarding.choose_hamster_and_fish(hamster="hamster-1", fish="fish-1", hamster_name="My Ham-StAr")
    app.onboarding.skip_video()
    app.onboarding.add_child()
    ##### (grade Levels' IDs: [toddler-time] [preschool] [pre-k] [kindergarten] [first-grade] [second-grade]
    app.onboarding.create_child_profile(child_name="Kevin", gender="M", academic_level="first-grade")
